...
for (i in 1:nrow(ws_today)) {
  ...
  if (instr_html != "") print(HTML(paste0("<p><strong>", format_out[[ws_today$format[i]]][2], ":</strong> ", instr_html)))
  ...
  if (params$render_zoom && !is.na(zoom_url_val)) {
    ...
    if (zoom_url_val %in% zoomlinks_tbl$abbrev) {
      ...
      if (toupper(zoom_lkp$txt) == "SEE EMAIL") {
        ...
        zoom_html <- paste0("<em>See your confirmation email</em>", reg_html)
        
      } else if (toupper(zoom_lkp$txt) == "URL") {
        
        zoom_html <- paste0("<a href='", zoom_lkp$mtg_url, 
                                "' target='_blank' rel='noopener'>", zoom_lkp$name, "</a>") 
    ...
    print(HTML(paste0("<p><strong>Zoom</strong>: ", zoom_html, "</p>")))
  ...
  if (!is.na(ws_today$assignment_name[i])) {
    ...
    print(HTML(paste0("<strong>Assignment for students<span title='Students taking DroneCamp as part of a course should consult with their instructor whether they should complete the assignment'><img src='images/info-icon_green_16x16x256.png' style='width:16px; height:16px; margin-bottom:3px;'/></span>:</strong> ", a1, ws_today$assignment_name[i], a2)))
