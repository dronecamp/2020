...
WebODM Lightning is a lightweight GUI for Windows. It also stitches images into 2D and 3D products, but the data crunching occurs elsewhere (a server running in the cloud, your network, or your computer).
...
If you have a working license for ArcGIS Pro, it almost certainly includes access to ESRI's online resources (which ArcGIS Pro uses also). But the reverse is not always true. An online account may or may not activate ArcGIS Pro. Check the details. 
</div>
...
To download the WebODM Lightning app for Windows, as well as create your *WebODM Lightning* account, please visit [**webodm.net**](https://webodm.net/){target="_blank" rel="noopener"}. You'll be all set for DroneCamp.

If you'd like to install OpenDroneMap your own hardware (and access it through a browser), you're welcome to try that also. See [OpenDroneMap.org](https://www.opendronemap.org/webodm/){target="_blank" rel="noopener"} for details.
