...
cols_wkshp1 <-c("dt_readable" = "Workshop #1 - Date & Time", 
                ...
                "track_namedt" = "Workshop #1 - Track", 
                "zoom_url" = "Workshop #1 - Zoom URL", 
                "facilitator" = "Workshop #1 - Zoom Facilitator", 
                "wkshp_dtsort" = "SortableTimeSlot1")

cols_wkshp2 <-c("dt_readable" = "Workshop #2 - Date & Time", 
                "venue" = "Workshop #2 - Venue", 
                "track_namedt" = "Workshop #2 - Track", 
                "zoom_url" = "Workshop #2 - Zoom URL", 
                "facilitator" = "Workshop #2 - Zoom Facilitator", 
                "wkshp_dtsort" = "SortableTimeSlot2")


ws1 <- dc20_wkshps_tbl %>% select(all_of(c(cols_core, cols_wkshp1)))
ws2 <- dc20_wkshps_tbl %>% select(all_of(c(cols_core, cols_wkshp2))) %>% filter(!is.na(wkshp_dtsort))
ws_all <- ws1 %>% bind_rows(ws2) %>% mutate(session_date = as.Date(substr(wkshp_dtsort, 1, 10)))
...
# names(ws_all2)
# View(ws_all2)
# ws_all2$wkshp_dtsort
# ws_all2$track_dtsort
...
# View(ws_all)
# ws_all2$track_dtsort
# $track_dtsort[is.na(ws_all2$track_dtsort), ] 
